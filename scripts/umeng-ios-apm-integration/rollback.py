...
class RollbackManager:
    ...
    def rollback(self, backup_path):
        ...
        print("  ⚠️  注意: 回滚会覆盖APM集成后的所有改动（含其他SDK修改）")
        print("  ⏳ 正在恢复...\n")
    ...
    def _zip_directory(self, directory, zipf):
        ...
        for root, dirs, files in os.walk(directory):
            # 目录级剪枝
            dirs[:] = [d for d in dirs if d not in ('Pods', 'backups', '.git')]
            # 跳过备份目录和Pods目录（太大）
            if 'backups' in root or '/Pods/' in root or root.endswith('/Pods'):
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description='iOS APM项目回滚工具')
    ...
    if args.list:
        ...
        print("\n此操作将覆盖当前项目的所有修改！")
        print("⚠️  如果在APM集成后又进行了其他SDK集成，回滚将覆盖这些改动。")
