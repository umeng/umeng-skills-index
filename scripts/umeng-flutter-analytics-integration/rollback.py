...
class RollbackManager:
    ...
    def rollback(self, backup_path=None, full=False):
        """执行回滚
        
        Args:
            backup_path: 备份文件路径，如果为None则自动查找最新备份
            full: True=全量回滚（删除目录+解压），False=仅回滚关键文件（默认）
        """
        if not backup_path:
    ...
    def rollback_files_only(self, backup_path=None):
        """仅回滚集成修改的关键文件（默认模式）"""
        if not backup_path:
            backup_path = self._find_latest_backup()
        ...
        try:
            ...
            with zipfile.ZipFile(backup_path, 'r') as zipf:
                for key_file in key_files:
                    # 在备份中查找对应文件
                    for zip_entry in zipf.namelist():
                        if zip_entry.endswith(key_file):
                            # 提取到正确位置
                            target_path = os.path.join(self.project_path, key_file)
                            # 确保目标目录存在
                            os.makedirs(os.path.dirname(target_path), exist_ok=True)
                            # 直接从备份恢复（无条件，支持恢复已被删除的文件）
                            with zipf.open(zip_entry) as src:
                                with open(target_path, 'wb') as dst:
                                    dst.write(src.read())
                            restored += 1
                            print("  ✅ 已恢复: {}".format(key_file))
                            break
            
            print("\n  共恢复 {} 个关键文件".format(restored))
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Flutter项目回滚工具')
    parser.add_argument('--project-path', required=True, help='项目路径')
    parser.add_argument('--backup-file', help='备份文件路径（用于回滚）')
    parser.add_argument('--list', action='store_true', help='列出所有备份')
    parser.add_argument('--full', action='store_true', help='全量回滚模式（默认为关键文件回滚）')
    ...
    if args.list:
        ...
        # 执行回滚
        mode_desc = "全量回滚" if args.full else "关键文件回滚"
        print("\n⚠️  确认回滚操作 ({})".format(mode_desc))
