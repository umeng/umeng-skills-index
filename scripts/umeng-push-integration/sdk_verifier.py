#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
推送SDK验证模块
通过logcat日志验证deviceToken获取成功
"""

import os
import subprocess
import time
import re
from typing import Tuple


class PushSDKVerifier:
    """推送SDK验证器"""
    
    def __init__(self, project_path: str, app_module: str):
        self.project_path = os.path.abspath(project_path)
        self.app_module = app_module
    
    def verify(self) -> Tuple[bool, str]:
        """
        验证推送SDK集成
        
        Returns:
            (是否成功, 详细信息)
        """
        print("\n🔍 开始验证推送SDK集成...\n")
        
        try:
            # 1. 检查设备
            device = self._check_device()
            if not device:
                return False, "无可用设备"
            
            # 2. 安装APK
            if not self._install_apk(device):
                return False, "APK安装失败"
            
            # 3. 启动应用
            if not self._launch_app(device):
                return False, "应用启动失败"
            
            # 4. 抓取logcat日志
            logs = self._capture_logcat(timeout=30)
            
            # 5. 分析日志
            return self._analyze_logs(logs)
            
        except Exception as e:
            return False, f"验证过程出错: {str(e)}"
    
    def _check_device(self) -> str:
        """检查可用设备"""
        print("步骤 1/4: 检查设备")
        
        try:
            result = subprocess.run(
                ['adb', 'devices', '-l'],
                capture_output=True,
                text=True,
                timeout=10
            )
            
            devices = []
            for line in result.stdout.split('\n')[1:]:
                if line.strip() and not line.startswith('*'):
                    parts = line.split()
                    if len(parts) >= 2 and parts[1] == 'device':
                        devices.append(parts[0])
            
            if not devices:
                print("  ❌ 未检测到可用设备")
                print("  请连接Android设备或启动模拟器")
                return None
            
            device = devices[0]
            print(f"  ✅ 检测到设备: {device}\n")
            return device
            
        except FileNotFoundError:
            print("  ❌ 未找到adb工具")
            return None
        except Exception as e:
            print(f"  ❌ 设备检测失败: {str(e)}")
            return None
    
    def _install_apk(self, device: str) -> bool:
        """安装APK"""
        print("步骤 2/4: 安装APK")
        
        # 查找APK文件
        apk_path = os.path.join(
            self.project_path,
            self.app_module,
            'build',
            'outputs',
            'apk',
            'debug',
            f'{self.app_module}-debug.apk'
        )
        
        if not os.path.exists(apk_path):
            print(f"  ❌ 未找到APK文件: {apk_path}")
            print("  请先编译项目")
            return False
        
        try:
            print(f"  安装APK: {os.path.basename(apk_path)}")
            result = subprocess.run(
                ['adb', '-s', device, 'install', '-r', apk_path],
                capture_output=True,
                text=True,
                timeout=60
            )
            
            if result.returncode == 0:
                print("  ✅ APK安装成功\n")
                return True
            else:
                print(f"  ❌ APK安装失败: {result.stderr}")
                return False
                
        except Exception as e:
            print(f"  ❌ APK安装失败: {str(e)}")
            return False
    
    def _launch_app(self, device: str) -> bool:
        """启动应用"""
        print("步骤 3/4: 启动应用")
        
        try:
            # 获取包名(从AndroidManifest.xml解析)
            package_name = self._get_package_name()
            if not package_name:
                print("  ❌ 无法获取应用包名")
                return False
            
            print(f"  启动应用: {package_name}")
            result = subprocess.run(
                ['adb', '-s', device, 'shell', 'monkey', '-p', package_name, '-c', 'android.intent.category.LAUNCHER', '1'],
                capture_output=True,
                text=True,
                timeout=20
            )
            
            if result.returncode == 0:
                print("  ✅ 应用启动成功\n")
                return True
            else:
                print(f"  ❌ 应用启动失败: {result.stderr}")
                return False
                
        except Exception as e:
            print(f"  ❌ 应用启动失败: {str(e)}")
            return False
    
    def _get_package_name(self) -> str:
        """从AndroidManifest.xml获取包名"""
        manifest_path = os.path.join(
            self.project_path,
            self.app_module,
            'src',
            'main',
            'AndroidManifest.xml'
        )
        
        if not os.path.exists(manifest_path):
            return None
        
        try:
            with open(manifest_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # 解析package属性
            match = re.search(r'package="([^"]+)"', content)
            if match:
                return match.group(1)
            
            return None
        except:
            return None
    
    def _capture_logcat(self, timeout: int = 30) -> str:
        """抓取logcat日志"""
        print(f"步骤 4/4: 抓取logcat日志(超时{timeout}秒)")
        
        try:
            # 清空logcat
            subprocess.run(['adb', 'logcat', '-c'], capture_output=True, timeout=5)
            
            # 等待应用初始化
            time.sleep(3)
            
            # 抓取日志
            result = subprocess.run(
                ['adb', 'logcat', '-d'],
                capture_output=True,
                text=True,
                timeout=timeout
            )
            
            logs = result.stdout
            print(f"  ✅ 已抓取 {len(logs)} 字符的日志\n")
            return logs
            
        except Exception as e:
            print(f"  ❌ 日志抓取失败: {str(e)}")
            return ""
    
    def _analyze_logs(self, logs: str) -> Tuple[bool, str]:
        """分析日志"""
        print("分析日志...\n")
        
        if not logs:
            return False, "未获取到日志"
        
        # 成功关键词
        success_patterns = [
            r'device_token["\s:]+([a-zA-Z0-9_-]{20,})',  # device_token字段，长度20+
            r'deviceToken:\s*([a-f0-9]{44})',  # 44位deviceToken
            r'Register success',
            r'register success'
        ]
        
        # 失败关键词（关键错误）
        error_keywords = [
            'message_secret is null',
            'PushAgent init failed',
            'messageSecret is empty',
            'Appkey、MessageSecret cannot be empty'
        ]
        
        # 警告关键词（可能失败，但不一定）
        warning_keywords = [
            'register failed',  # 可能是网络问题，不代表配置错误
        ]
        
        # 检查失败关键词
        for keyword in error_keywords:
            if keyword in logs:
                print(f"❌ 检测到错误关键词: {keyword}")
                print("\n可能原因:")
                if 'message_secret' in keyword or 'messageSecret' in keyword or 'Appkey' in keyword:
                    print("  - messageSecret配置错误或为空")
                    print("  - 请检查AndroidManifest.xml中的UMENG_MESSAGE_SECRET配置")
                elif 'init failed' in keyword:
                    print("  - PushAgent初始化失败")
                
                return False, f"检测到错误: {keyword}"
        
        # 检查警告关键词
        for keyword in warning_keywords:
            if keyword in logs:
                print(f"⚠️  检测到警告关键词: {keyword}")
                print("  可能是网络问题，不影响SDK集成正确性")
                # 不立即返回失败，继续检查是否有成功关键词
        
        # 检查成功关键词
        for pattern in success_patterns:
            match = re.search(pattern, logs, re.IGNORECASE)
            if match:
                if 'device_token' in pattern or 'deviceToken' in pattern:
                    device_token = match.group(1)
                    print(f"✅ 检测到deviceToken: {device_token}")
                    print("✅ 推送SDK集成成功!\n")
                    return True, f"deviceToken获取成功: {device_token}"
                else:
                    print(f"✅ 检测到成功关键词: {match.group(0)}")
                    print("✅ 推送SDK集成成功!\n")
                    return True, "推送注册成功"
        
        # 未找到任何关键词
        print("⚠️  未检测到成功或失败关键词")
        print("\n可能原因:")
        print("  - 应用启动时间不足,SDK尚未完成初始化")
        print("  - 网络连接问题")
        print("  - messageSecret配置错误")
        print("\n建议:")
        print("  1. 手动查看logcat日志:")
        print("     adb logcat | grep -E 'device_token|deviceToken|sendLaunch'")
        print("  2. 检查Application类中的messageSecret配置")
        print("  3. 检查AndroidManifest.xml中的UMENG_MESSAGE_SECRET配置")
        print("  4. 确保设备网络正常\n")
        
        return False, "未检测到deviceToken,需要手动验证"
